...
export type AgentCapability = 'memory' | 'voice' | 'collaboration' | 'analytics' | 'research' | 'creative' | 'support' | 'automation';
export type CollaborationMode = 'expert_panel' | 'debate' | 'task_chain' | 'parallel';
export type MemoryLevel = 'basic' | 'standard' | 'advanced' | 'unlimited';
export type ResponseStyle = 'professional' | 'casual' | 'technical' | 'creative' | 'supportive';
...
export const mockAgents: Agent[] = [
  {
    ...
    config: {
      ...
      collaboration: { enabled: true, modes: ['expert_panel', 'task_chain'], maxTeamSize: 5 },
      analytics: { enabled: true, trackInteractions: true, generateReports: true }
    },
    personality: {
      tone: 'professional',
    ...
    integrations: {
      apiConnections: [
        { id: 'api_1', name: 'Google Analytics', type: 'analytics', enabled: true },
        { id: 'api_2', name: 'Tableau', type: 'visualization', enabled: true }
      ],
      dataSources: [
        { id: 'ds_1', name: 'Sales Database', type: 'database', enabled: true },
        { id: 'ds_2', name: 'Marketing API', type: 'api', url: 'https://api.marketing.com', enabled: true }
      ],
      permissions: {
  ...
  },
  {
    ...
    description: '24/7 customer service automation agent with multilingual support',
    avatar: '🎧',
    status: 'active',
    ...
    personality: {
      tone: 'supportive',
      expertiseAreas: ['Customer Service', 'Product Knowledge', 'Troubleshooting'],
